...
file.dir <- c("/efs/garcia/users/kd2630/noncoding_telo/STAAR/MetaSTAAR_Discovery_TOPMed/Discovery/Gene_Centric_Coding_Analysis_noncod_adj_null_mod/",
              "/efs/garcia/users/kd2630/noncoding_telo/STAAR/MetaSTAAR_Discovery_TOPMed/TOPMed/Gene_Centric_Coding_Analysis_noncod_adj_null_mod/")
...
## output path
output_path <- "/efs/garcia/users/kd2630/noncoding_telo/STAAR/MetaSTAAR_Discovery_TOPMed/Coding_Meta_Analysis_noncod_adj_null_mod/"
...
for (gn in genes_to_analyze) {
	...
	message("Chr: ", chr)
	  ...
	  coding_sumstat_gene_list <- lapply(sumstat.file.path, function(x) {
	    coding_sumstat_list[[paste0(x,".coding_sumstat")]][[gn]]
	  })
	  coding_cov_gene_list <- lapply(cov.file.path, function(x) {
	    coding_cov_list[[paste0(x,".coding_cov")]][[gn]]
	  })
